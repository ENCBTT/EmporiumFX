...
public class CMesas implements Initializable {
    //columnas tabla
    @FXML
    private TableColumn<MMesas, Integer> cl1;
    @FXML
    private TableColumn<MMesas, String> cl2;
    @FXML
    private TableColumn<MMesas, MEstados> cl3;
    //Componentes GUI
    @FXML
    ...
    @FXML
    private TableView<MMesas> tb_principal;
    @FXML
    private ComboBox<MEstados> cb_1;

    //collecion
    ObservableList<MMesas> listaTbMun;
    ObservableList<MEstados> listaCb1;
    //Logger
    private static final Logger LOGGER = LoggerFactory.getLogger(CMesas.class.getSimpleName());
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        //llamar metodos
        MMesas.obtenerInfoMun(conexion.getConnection(), listaTbMun);
        MEstados.obtenerInfoMun(conexion.getConnection(), listaCb1);
        //cargar tabla
        tb_principal.setItems(listaTbMun);
        //Enlazar con atributos
        //Columnas
        cl1.setCellValueFactory(new PropertyValueFactory<MMesas, Integer>("codMun"));
        cl2.setCellValueFactory(new PropertyValueFactory<MMesas, String>("desMun"));
        cl3.setCellValueFactory(new PropertyValueFactory<MMesas, MEstados>("estado"));
        //ComboBox
    ...
    public void gestionar_eventos() {
        tb_principal.getSelectionModel().selectedItemProperty().addListener(new ChangeListener<MMesas>() {
            @Override
            public void changed(ObservableValue<? extends MMesas> observableValue, MMesas valorAnterior, MMesas valorSeleccionado) {
                if (valorSeleccionado != null) {
                    txt_cod.setText(String.valueOf(valorSeleccionado.getCodMun()));
                    txt_des.setText(valorSeleccionado.getDesMun());
                    cb_1.setValue(valorSeleccionado.getEstado());
    ...
    public void guardarRegistro() {
        ...
        //crea una nueva instancia del tipo mesas
        MMesas a = new MMesas(ultCod(), txt_des.getText(),cb_1.getSelectionModel().getSelectedItem());
        ...
        if(!validacion()){
            ...
            //abre conexion
            conexion.iniciarConexion();
            //llamar al metodo guardar de la clase mesas
    ...
    public void editarRegistro() {
        ...
        Alert mensaje = new Alert(Alert.AlertType.INFORMATION);
        //crea una nueva instancia del tipo mesas
        MMesas a = new MMesas(Integer.valueOf(txt_cod.getText()), txt_des.getText(), cb_1.getSelectionModel().getSelectedItem());
        if(!validacion()){
            LOGGER.info("Informaciones no validas");
        } else {
            //abre conexion
            conexion.iniciarConexion();
            //llamar al metodo editar de la clase mesas
    ...
    public void busqueda() {
        (new Thread(() -> {
            ...
            MMesas.busquedaInfoMun(conexion.getConnection(),listaTbMun, filtro);
            //cerrar conexion
            conexion.cerrarConexion();
        })).start();
    }
    ...
    public void imprimir(){
        (new Thread(() -> {
            String rt = CMesas.this.getClass().getSimpleName();
    ...
    public boolean validacion(){
        if(this.txt_des.getText().isEmpty() || this.txt_des == null){
            ...
            mensaje.setTitle("Faltan datos");
            mensaje.setContentText("");
            mensaje.setHeaderText("Falta seleccionar estado");
            mensaje.show();
            return false;
        }else {
            return true;
